        ...
        private void sign_in_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if ((name.Text.Length != 0) && (id.Text.Length != 0) && (user_name.Text.Length != 0) && (password.Text.Length != 0))
                {
                    ...
                    if (check == "0")
                    {
                        string command = "INSERT INTO `user`(`id_user`, `name`, `username`, `password`, `position`, `state`) VALUES (" + int.Parse(id.Text.ToString()) + ",'" + name.Text + "','" + user_name.Text + "','" + password.Text + "','User',0)";
                        MessageBox.Show(command);
                        string add_user = adm.data_user(command);

                        if (add_user == "0")
                        {
                            MessageBox.Show("تمت اضافة مشرف جديد في النظام");
                            id.Clear();
                            name.Clear();
                            password.Clear();
                            user_name.Clear();
